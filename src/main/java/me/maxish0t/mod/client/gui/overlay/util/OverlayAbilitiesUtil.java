...
public class OverlayAbilitiesUtil {
    ...
    public static void renderSuperBreakerBox(String abilityName, String abilityIconName, PoseStack poseStack, double posX, double posY) {
        ...
        if (ClientMiningEvents.unlockedSuperBreaker) {
        ...
        if ((int)UpdateLevelPacket.level < (int) ClientMiningEvents.SUPER_BREAKER_AMOUNT) {
            RenderUtil.drawRectangle(statsBar, Math.round(ModUtil.calculatePercentageFloat((int)UpdateLevelPacket.level, (int) ClientMiningEvents.SUPER_BREAKER_AMOUNT)),  8, 2, 2, 1006063360);
        }
